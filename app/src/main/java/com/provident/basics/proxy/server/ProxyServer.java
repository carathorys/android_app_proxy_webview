    ...
    private class ProxyConnection implements Runnable {
        ...
        public void run() {
            try {
                ...
                if (server != null) {
                    SocketConnect.connect(connection, server, new ISocketDataResponse() {
                        @Override
                        public void add(long l) {
                            Log.d(TAG, String.format("add inbound bytes: %1s", l));
                    ...
                    }, new ISocketDataResponse() {
                        @Override
                        public void add(long l) {
                            Log.d(TAG, String.format("add outbound bytes: %1s", l));
