...
import com.provident.proxylibrary.ISocketDataResponse;
...
public class ProxyServer extends Thread {
    ...
    private long REQUEST_DATA_INBOUND_SIZE = 0;

    private long REQUEST_DATA_OUTBOUND_SIZE = 0;

    private ExecutorService threadExecutor;
    private ServerSocket serverSocket;
    private int mPort;
    private IProxyPortListener mCallback;
    private IRequestUpdateNotification mRequestNotiUpdate;
        ...
        try {
            ...
            while (mIsRunning) {
                try {
                    ...
                    if (mRequestNotiUpdate != null) {
                        mRequestNotiUpdate.requestNotificationUpdate(REQUEST_COUNTER, REQUEST_DATA_INBOUND_SIZE, REQUEST_DATA_OUTBOUND_SIZE);
                    }
                    REQUEST_COUNTER++;
    ...
    public synchronized void setNotificationUpdated(IRequestUpdateNotification callback) {
        mRequestNotiUpdate = callback;
    }
    ...
    private class ProxyConnection implements Runnable {
        ...
        public void run() {
            try {
                ...
                if (server != null) {
                    SocketConnect.connect(connection, server, new ISocketDataResponse.Stub() {
                        @Override
                        public void add(long l) throws RemoteException {
                            REQUEST_DATA_INBOUND_SIZE += l;
                            try {
                                mRequestNotiUpdate.requestNotificationUpdate(REQUEST_COUNTER, REQUEST_DATA_INBOUND_SIZE, REQUEST_DATA_OUTBOUND_SIZE);
                            } catch (RemoteException e) {
                                e.printStackTrace();
                    ...
                    }, new ISocketDataResponse.Stub() {
                        @Override
                        public void add(long l) throws RemoteException {
                            REQUEST_DATA_OUTBOUND_SIZE += l;
                            try {
                                mRequestNotiUpdate.requestNotificationUpdate(REQUEST_COUNTER, REQUEST_DATA_INBOUND_SIZE, REQUEST_DATA_OUTBOUND_SIZE);
                            } catch (RemoteException e) {
                                e.printStackTrace();
        ...
        private String getLine(InputStream inputStream) throws IOException {
            ...
            REQUEST_DATA_INBOUND_SIZE += buffer.toString().getBytes().length;
        ...
        private void sendLine(Socket socket, String line) throws IOException {
            ...
            os.write('\r');
            os.write('\n');
            REQUEST_DATA_OUTBOUND_SIZE += bytes.length + 3;
            os.flush();
        }
